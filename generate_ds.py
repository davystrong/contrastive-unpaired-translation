...
ds = AP_jp2('/users/40390351/sharedscratch/AP_jp2s', 286, step=256, downsample=4, transform=RemoveBlack(), target_transform=RemoveBlack())
    ...
    for name, x in zip(['A', 'B'], pair):
        ...
        if np.all(blacks):
            ...
            if np.any(blacks):
                ...
                med = np.median(x[border], axis=0)
                x = np.where(blacks, med.astype(np.uint8), x)
            path = Path(f'datasets/mouse/train{name}')
